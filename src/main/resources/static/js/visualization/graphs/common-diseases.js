...
let simulation = d3.forceSimulation(graph.nodes)
    .force("link", d3.forceLink().id(d=>d.id))
    .force("charge", d3.forceManyBody().strength(d=>d.type==="disease"?diseasesN:singleNodeStrength).distanceMin(nodeRadius*2).distanceMax(height))
    .force("collide", d3.forceCollide().radius(d=> {
        return (d.degree <= noLabelDegree||d.depth===1 ? 1.5 : (d.type==="disease"?10: 5)) * nodeRadius

    }))

if (diseasesN > height / 25) {
    simulation
...
let node = svg.append("g")
    ...
    .enter()
    .append("g")
    .attr("class", d => d.intersection.map(i => 'intersection_' + i).join(" ").concat(" menu"))
...
let label = node.append("text")
    .text(d => d.type === "disease" ? d.name : (d.degree > noLabelDegree || diseasesN === 1 || noIntersections ? d.name : "")) // No labels in nodes with degree === noLabelDegree (looks messy)
    .attr("id", d => d.id)
...
circle.on("mouseover", function () {
    if (parseInt(this.getAttribute("data-degree")) <= noLabelDegree && !noIntersections) { //parseInt and not addition operator "+" bc null needs to be null
...
function ticked() {
    ...
    // circle.filter(c=>c.type==="disease").each(d=>{h = height/2; let min = height/(diseasesN+1); d.y -= ((d.y-(h/(d.y/height))))})
    //display circles so that their label is not off range
    circle
        .attr("cx", d => {
            let textSize = (+d.degree === noLabelDegree && diseasesN !== 1) ? 0 : +d3.select("#" + d.id).attr('data-text-length')
            d.x = Math.max(nodeRadius, Math.min(width - nodeRadius*4 - textSize, d.x));
            return d.x
        })
        .attr("cy", d => {
            d.y = Math.max(nodeRadius+(d.type==="disease"?nodeRadius*2:0), Math.min(height - nodeRadius*2, d.y));
            return d.y
        });
// .attr("cx", d => {
